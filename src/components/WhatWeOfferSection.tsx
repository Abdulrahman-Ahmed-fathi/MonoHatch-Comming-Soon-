...
const itemKeys = ["ai", "care", "learn", "tracking"] as const;
const itemImages = { ai: feature1, care: feature2, learn: feature3, tracking: feature4 };
          ...
          {itemKeys.map((key, i) => (
            ...
            >
              <LandingCard hover className="h-full border border-white/40 bg-white/70 p-5 transition-all duration-300 hover:-translate-y-1">
                ...
                <p className="p2-r mt-2 text-sm text-[#6F4A65]">{t(`features.items.${key}.body`)}</p>
              </LandingCard>
            </motion.div>
          ))}
        </div>
